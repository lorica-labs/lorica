...
//! and is a floor this project cannot ask for: the kernel floor here is 6.8. Emulating
//! one with a bounded loop over an array puts the whole cost back and adds the masked
//! index the verifier refuses to trust. So this file states the shape and returns
//! nothing, which is a claim the compiler checks — `backend/mod.rs` holds it to the
//! trait on every build — rather than a `todo!()` that is a debt with a nicer name.
//!
