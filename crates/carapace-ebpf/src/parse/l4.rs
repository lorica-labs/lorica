    ...
    match l3.proto {
        IPPROTO_TCP => {
            ...
            Ok(L4 {
                ...
                tcp_flags: hdr[13],
                // A data offset below five describes a header shorter than its own
                // fixed part. It is not refused here — no verdict has ever been stated
                // on it — but the payload offset is floored, so a signature read can
                // never land back inside the header.
                hdr_len: (hdr[12] >> 4).max(TCP_MIN_WORDS) as usize * 4,
                ..L4::NONE
            })
        }
        IPPROTO_UDP => {
