...
//! duplication anyone can remove by refactoring. `carapace-ebpf` is a `no_std` crate
//! for another target in a workspace of its own, so nothing here can link against it,
//! and the only vocabulary the two sides share is what `carapace-common` freezes:
...
use carapace_common::{Action, CounterId};
