...
const DEFAULT_SOCKET: &str = "/run/carapace/control.sock";
const PROGRAM: &str = "carapace_xdp";

const USAGE: &str = "usage: carapaced --object PATH [--socket PATH] [--counters N] \
        ...
        tokio::select! {
            _ = timer.tick() => {
                ...
                if deadline.is_some_and(|at| tokio::time::Instant::now() >= at) {
                    eprintln!(
                        "carapaced: {} ticks, {} full sweeps of {} slots, {} failed, \
