...
use lorica_common::{NO_CUTOFF, STAGE_CUTOFF_SHIFT};
use support::{
...
fn only_level() -> Option<u32> {
    let raw = std::env::var("LORICA_STAGE_CUTOFF").ok()?;
    let stages = raw
        .trim()
        .parse::<u32>()
        .unwrap_or_else(|_| panic!("LORICA_STAGE_CUTOFF is {raw:?}, which is not a level"));
    assert!(
        LEVELS.iter().any(|(s, _)| *s == stages),
        "LORICA_STAGE_CUTOFF is {stages}, and the pipeline has {} levels",
        LEVELS.len()
    );
    Some(stages)
}
...
fn one_level_of_the_pipeline_under_a_profiler() {
    let Some(stages) = only_level() else {
        println!("LORICA_STAGE_CUTOFF unset, nothing to profile");
        return;
    };
    let label = LEVELS
        .iter()
...
fn passes() -> usize {
    match std::env::var("LORICA_STAGE_PASSES") {
        Ok(raw) => raw
            .trim()
            .parse()
            .unwrap_or_else(|_| panic!("LORICA_STAGE_PASSES is {raw:?}, which is not a count")),
