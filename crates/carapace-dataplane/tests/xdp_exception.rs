...
fn deliberate_drops_under_real_traffic_raise_no_exception() {
    let (link, near) = Link::wired("cara-exc", 77);
    ...
    let _attached = carapace_dataplane::loader::attach_native(program, &link.name)
...
fn read_counter(ebpf: &aya::Ebpf, name: &str) -> u64 {
    ...
    let id = carapace_common::CounterId::from_name(name)
