...
//! that length is tied to a map declared in `carapace-ebpf`, so a caller never has to
...
pub fn counters<'a>(
    ...
    // SAFETY: COUNTERS is declared `PerCpuArray<u64>` in carapace-ebpf, which is the map
