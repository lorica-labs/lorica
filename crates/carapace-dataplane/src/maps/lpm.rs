...
use carapace_common::{LpmKey, LpmValue};

use super::batch;

/// The kernel strides a batch buffer by the map's own key size, and for an `LPM_TRIE`
/// that size is the `u32` prefix length in front of the address. `carapace-ebpf`
...
pub fn load(fd: BorrowedFd<'_>, entries: &[(LpmKey, LpmValue)], chunk: usize) -> io::Result<()> {
    ...
    for group in entries.chunks(chunk) {
        ...
        // with in carapace-ebpf, so their sizes are the map's key and value sizes. The
