...
//! without telling anyone, and being replaced in turn would stop Carapace the same way.
...
pub enum AttachError {
    #[error(
        "the XDP hook of {iface} is already held by {occupant}; refusing to replace it. \
         ...
         Carapace on another interface."
    )]
    Occupied { iface: String, occupant: String },

    #[error(
        "{iface} has no native XDP support, and Carapace does not fall back to generic \
